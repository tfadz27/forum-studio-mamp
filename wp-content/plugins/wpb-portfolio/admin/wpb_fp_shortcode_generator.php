...
function wpb_fp_shortcode_generator_metaboxes(){
	...
	$fields = array(
		...
		array(
			...
			'desc'	=> __( 'Default: off. Check it for showing the Pagination', WPB_FP_TEXTDOMAIN ),
			'id'	=> $prefix.'pagination',
			'type'	=> 'checkbox'
		),
		array(
			'label'	=> __( 'Need Filtering', WPB_FP_TEXTDOMAIN ),
			'desc'	=> __( 'Default: On. Check it for disable the Filtering', WPB_FP_TEXTDOMAIN ),
			'id'	=> $prefix.'filtering',
			'type'	=> 'checkbox'
		),
		array(
...
if( !function_exists('wpb_fp_generated_shortcode_function') ):
	function wpb_fp_generated_shortcode_function( $atts ){
		...
		$pagination = $pagination ? 'pagination="on"' : '';

		$filtering = get_post_meta( $id, 'wpb_fp_filtering', true );
		$filtering = $filtering ? 'filtering="no"' : 'filtering="yes"';
